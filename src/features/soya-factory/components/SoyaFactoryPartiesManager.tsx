...
import {
  createSoyaFactoryPartyAction,
  createSoyaFactoryPartyPaymentAction,
  deleteSoyaFactoryPartyAction,
  deleteSoyaFactoryPartyPaymentAction,
  updateSoyaFactoryPartyAction,
...
interface SoyaFactoryPartiesManagerProps {
  parties: BiltyParty[];
  biltys: Bilty[];
...
/**
 * Party master + payment ledger for Soya Factory.
 *
 * Copied once from the maize BiltyPartiesManager rather than extracted out of
 * it: that component is live revenue-carrying code with no test coverage, and
 * the brief is that the maize side must not be touched. The record types stay
 * the maize ones (see soya-factory.service.ts) so the statement view is reused
 * verbatim.
 */
export function SoyaFactoryPartiesManager({
  parties,
  biltys,
  payments,
}: SoyaFactoryPartiesManagerProps) {
  ...
  const handleSaveParty = () => {
    ...
    startTransition(async () => {
      try {
        if (editingPartyId) {
          const updated = await updateSoyaFactoryPartyAction(
            editingPartyId,
            value,
          ...
          const created = await createSoyaFactoryPartyAction(
            value,
            partyPlaceDraft.trim(),
            partyMobDraft.trim()
          );
  ...
  const handleDeleteParty = (party: BiltyParty) => {
    startTransition(async () => {
      try {
        await deleteSoyaFactoryPartyAction(party.id);
        setPartyData((current) => {
  ...
  const handleCreatePayment = () => {
    ...
    startTransition(async () => {
      try {
        const created = await createSoyaFactoryPartyPaymentAction({
  ...
  const handleDeletePayment = (payment: BiltyPartyPayment) => {
    startTransition(async () => {
      try {
        await deleteSoyaFactoryPartyPaymentAction(payment.id);
  ...
  const handleExportPartyPurchases = () => {
    ...
    exportRowsToCsv(rows.length > 0 ? rows : [{ "Party Name": activeParty.name }], {
      fileName: `soya-factory-party-${activeParty.name.replace(/\s+/g, "-").toLowerCase()}`,
        ...
        >
          ...
          {activeParty && statementOpen ? (
            <iframe
              ...
              src={`/soya/factory/parties/${activeParty.id}/statement?embed=1`}
