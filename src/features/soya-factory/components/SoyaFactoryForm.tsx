...
import { biltySchema, type Bilty } from "@/features/bilty/schemas";
import {
  checkSoyaFactoryBillNoAvailabilityAction,
  createSoyaFactoryPartyAction,
  createSoyaFactoryRecordAction,
  updateSoyaFactoryRecordAction,
...
type SoyaFactoryFormValues = z.input<typeof biltySchema>;

/**
 * Soya Factory entry form. The maize bilty record shape and its zod schema are
 * reused as-is; only the actions, routes and copy differ. There is no
 * bill-generation flow on the Soya side, so the maize form's linkedBillNo prop
...
export function SoyaFactoryForm({
  ...
  const form = useForm<SoyaFactoryFormValues>({
  ...
  async function handleAddParty() {
    ...
    try {
      const created = await createSoyaFactoryPartyAction(candidate, place, mob);
  ...
  async function onSubmit(values: SoyaFactoryFormValues) {
    setIsLoading(true);
    try {
      ...
      if (isEditing && initialData?.id) {
        await updateSoyaFactoryRecordAction(initialData.id, payload);
        toast.success("Entry Updated");
        router.replace("/soya/factory");
      } else {
        await createSoyaFactoryRecordAction(payload);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        const available = await checkSoyaFactoryBillNoAvailabilityAction(
          candidate,
          dateForCheck,
          ...
          >
            ...
            <div className="flex justify-end gap-3">
              <Button
                type="button"
                variant="outline"
                onClick={() => router.replace("/soya/factory")}
